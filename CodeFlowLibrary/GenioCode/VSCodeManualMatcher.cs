...
namespace CodeFlowLibrary.GenioCode
{
    public class VSCodeManualMatcher
    {
        ...
        private IManual DoMatch(Type matchType, ManualMatchProvider provider, int begin)
        {
            ...
            int i = VsCodeSnapshot.IndexOf(Util.Helpers.NewLine, idx, StringComparison.Ordinal);
            ...
            if (Guid.TryParse(guid, out Guid g))
            {
                var match = new ManualMatch
                {
                    ...
                    CodeStart = i + Util.Helpers.NewLine.Length
                };

                // Match line above begin tag
                var endUpperLine = VsCodeSnapshot.LastIndexOf(Util.Helpers.NewLine, begin, StringComparison.Ordinal);
                if (endUpperLine > -1)
                {
                    beginUpperLine = VsCodeSnapshot.LastIndexOf(Util.Helpers.NewLine, endUpperLine, StringComparison.Ordinal) +
                                     Util.Helpers.NewLine.Length;
                ...
                if (end == -1)
                {
                    ...
                    code = VsCodeSnapshot.Substring(i + Util.Helpers.NewLine.Length);
                }
                else
                {
                    int length = end - match.CodeStart;
                    var c = VsCodeSnapshot.Substring(match.CodeStart, length);
                    int tmp = c.LastIndexOf(Util.Helpers.NewLine, StringComparison.Ordinal);
                    length = tmp != -1 ? tmp : 0;
                ...
                if (anotherB == -1)
                {
                    ...
                    {
                        man.Code = ManualCodeOptions.ForceDOSLine ? Util.Helpers.ConverToDOSLineEndings(code)
                                : code;
                        man.CodeId = g;
